...
class Sample:
    def __init__(self, sampleFile, sampleLabel):
        ...
        self.spectralContrast = librosa.feature.spectral_contrast(y=self.y, sr=self.sr, n_bands=4)     # leaving this at ~1500 vals since it will probably correlate to replays
...
def getAverageValue(valuesList):
    sum = 0
    for x in valuesList:
        sum += x
    return sum / len(valuesList)

def getSetOfAverages(valuesList, numOfAverages):
    subListSize = int(len(valuesList)/numOfAverages)
    avgList = []
    for n in range(numOfAverages):
        subList = valuesList[n*subListSize:(n+1)*subListSize]
        subAvg = getAverageValue(subList)
        avgList.append(subAvg)
    return avgList
...
def constructAveragesHeader():
    header = []
    header.append("FileNum")
    header.append("FileName")
    for i in range(4):
        header.append("CC" + str(i+1))
    for i in range(4):
        header.append("MS" + str(i+1))
    for i in range(8):
        header.append("MFCC" + str(i+1))
    for i in range(4):
        header.append("RMS" + str(i+1))
    for i in range(2):
        header.append("SC" + str(i+1))
    for i in range(2):
        header.append("SB" + str(i+1))
    for i in range(8):
        header.append("SCO" + str(i+1))
    for i in range(2):
        header.append("SF" + str(i+1))
    for i in range(2):
        header.append("SR" + str(i+1))
    for i in range(8):
        header.append("TC" + str(i+1))
    for i in range(4):
        header.append("ZC" + str(i+1))
    header.append("SampleID")
    return header


# creates a row containing sets of average values for each feature
# number of average values should match number of header labels foe each value
def constructAveragesRow(sample):
    row = []
    for x in getSetOfAverages(listValuesPretty(sample.getChromaCens()), 4):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getMelSpect()), 4):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getMfcc()), 8):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getRms()), 4):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getSpectralCentroid()), 2):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getSpectralBandwidth()), 2):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getSpectralContrast()), 8):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getSpectralFlatness()), 2):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getSpectralRolloff()), 2):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getTonalCentroid()), 8):
        row.append(x)
    for x in getSetOfAverages(listValuesPretty(sample.getZeroCrossingRate()), 4):
        row.append(x)
    row.append(sample.name)
    return row
...
def writeTrainingDataToCsv(sampleParentFolderPath, numOfSamples):
    ...
    trainingDataOutput = open("trainingDataFinal.csv", "w+", encoding="utf8")
    c = 0
    ...
    for x in trainingFolderSet:
        ...
        if c == 0:
            writer.writerow(constructAveragesHeader())
        i = 1
        print(x)
        for sampleFile in fileNameList:
            if i <= numOfSamples:
                ...
                row.append(str(i))
                row.append(sampleFile)
                featureList = constructAveragesRow(audioSample)
                for y in featureList:
                    row.append(y)
...
def writeTestingDataToCsv(sampleParentFolderPath, numOfSamples):
    ...
    testingDataOutput = open("testingDataFinal.csv", "w+", encoding="utf8")
    c = 0
...
writeTestingDataToCsv("C:\\Users\\sydney\\Documents\\Recordings\\Replay-Recordings", 200)
